...
- [x] **SEM-02 (Combined Classification + Tagging Batch - implemented 2026-08-21)**: An admin-triggered batch tool (`admin/classify_sdgs_llm.php`, mirrors the Phase 6-9 shape: list-unclassified / process-one / graceful skip-on-error) sends each publication's title+abstract to the model in one call, requesting JSON with `sdg_primary`/`confidence_primary`/`sdg_secondary`/`confidence_secondary`/`rationale`/`semantic_tags` - one call produces both the SDG classification and the expert-finder's semantic tags, never computed live on a public page render. New columns via `database/add_llm_classification_columns.php` (`llm_sdg_primary`, `llm_confidence_primary`, `llm_sdg_secondary`, `llm_confidence_secondary`, `llm_rationale`, `llm_semantic_tags`, `llm_model`, `llm_checked_at`, `llm_reviewed_by`/`llm_reviewed_at` for SEM-08). `fetch_llm_sdg_classification()`/`_with_retry()` added to `includes/functions.php`, alongside a new `http_post_json()` helper (this project's HTTP helpers were GET-only until now). Verified end-to-end against the local Docker stack and the real UP AI Connect endpoint: publication #28 ("Dietary restriction, vegetarian diet, and aging intervention") → SDG 3 (95%) primary, SDG 2 (62%) secondary, sensible Thai rationale and semantic tags, all written to the DB; missing-CSRF request → 403; unauthenticated request → 401; re-querying `action=list` after processing correctly excluded #28. Test write reverted after verification, matching this project's established practice. *Frontend UI note (2026-08-21): the batch-trigger UI for this tool lives at `admin/sdg_llm.php` (built by a parallel server-side session, reconciled with this backend - see ROADMAP.md Coverage Notes), not inline in `sdg_import.php`.*
- [x] **SEM-03 (Confidence Score & Rationale Display - implemented 2026-08-21)**: The LLM's confidence score and rationale is shown next to each SDG badge on `reports.php` and `publications_search.php`, visually distinguished from the existing Phase 6 keyword-dictionary classification. New `render_llm_sdg_badge()` in `includes/functions.php`: dashed violet border + robot icon + confidence % (e.g. "SDG 3 (96%)"), deliberately not any SDG's own brand color, so it reads as "classifier badge" rather than an SDG-specific color. `publications_search.php` shows it compactly with the rationale as a hover tooltip (`title` attribute); `reports.php`'s SDG tab shows a full rationale box below the badges, labeled with the model name, alongside (not replacing) the existing green keyword-dictionary rationale box. Verified end-to-end against the local Docker stack: classified 2 real publications, confirmed both pages render the badge, confidence %, tooltip, and rationale box correctly; test data reverted after verification.
- [x] **SEM-04 (Runs Alongside, Not Replacing - satisfied by design)**: The existing Phase 6 keyword-dictionary classifier keeps running unchanged - the LLM classifier writes only to separate `llm_*` columns (never `sdg_primary`/`secondary`/`tertiary`), and both are displayed side by side (see SEM-03), so SEM-07's comparison is possible and a live classification isn't riding on an unvalidated model alone days before presenting.
- [x] **SEM-05 (LLM-Ranked Expert Finder - implemented 2026-08-21)**: A search box (`admin/expert_finder.php`) accepts a free-text question in Thai or English; one single chat-completion call (`fetch_llm_expert_ranking()`/`_with_retry()` in `includes/functions.php`, backed by `admin/expert_finder_llm.php`) sends the query plus every classified publication's `semantic_tags` and returns a ranked list of publication IDs with a relevance score/reason - the endpoint then looks up the real researchers linked to those IDs via `researcher_publications`/`researchers` (never trusting a researcher name the model might mention directly) and groups results by researcher. Only IDs that were actually offered to the model are ever trusted (defensive against hallucinated IDs). Verified end-to-end against the local Docker stack and the real UP AI Connect endpoint: classified 15 real publications, searched "nutrition and aging" → correctly ranked 4 real researchers by relevance with sensible Thai reasons and their actual matching publications (e.g. รศ.ดร. ศุภชัย เจริญสิน at 96% for the dietary-restriction/aging paper); a nonsense query ("quantum computing blockchain") correctly returned zero researchers rather than a false match. CSRF/auth checks verified (403/401). Test data reverted after verification.
- [x] **SEM-06 (Quota-Spread & Demo-Day Safeguard - implemented 2026-08-21)**: `database/add_expert_finder_cache.php` adds an `expert_finder_cache` table keyed by an exact-text hash of the query - a repeat of a rehearsed demo query is served instantly from this cache with zero UP AI Connect calls (verified live: first call 1.5s via the real API, identical repeat call 0.03s from cache - ~47x faster, byte-identical response), so the live presentation never depends on an uncached request succeeding on stage. The one-time SEM-02 batch (~209,000 tokens total, verified) still fits within a single provider's daily quota or can be split across providers if convenient.
- [~] **SEM-07 (Precision/Recall Validation - tooling built 2026-08-21, labeling not started)**: `database/add_sdg_gold_standard.php` (new `sdg_gold_standard` table) + `admin/sdg_validation.php` (blind labeling UI - deliberately shows only title/abstract, never either classifier's guess, to avoid anchoring the reviewer's judgment) + `admin/sdg_validation_action.php` (save one label, CSRF-protected) + a Precision/Recall/F1 computation comparing the Phase 6 keyword classifier against the Phase 10 LLM classifier. **This requires a real subject-matter reviewer to label 150-200 publications by hand - that part cannot be done by an AI without producing a fabricated, meaningless metric, and was deliberately not attempted here.** Verified the tooling itself end-to-end against the local Docker stack: submitted 3 real blind labels (2 deliberately matching the LLM's actual guess, 1 deliberately not) and confirmed the computed Precision/Recall/F1 came out exactly as expected (66.7% for the LLM classifier, N/A for the keyword classifier since it hadn't classified those 3 publications yet) - proves the math is correct, not that any real accuracy claim exists yet. Test labels reverted after verification. **Next action needed from a human on the team**: run "LLM Classify" across at least 150-200 publications, then have a qualified reviewer label a random sample at `admin/sdg_validation.php` before this number can be quoted to the committee.
- [x] **SEM-08 (Human Review / Audit Trail - implemented 2026-08-21)**: `admin/llm_review.php` lists LLM classifications below a documented confidence threshold (`MIN_LLM_REVIEW_CONFIDENCE = 70`) that haven't been reviewed yet, ordered least-confident first; `admin/llm_review_action.php` lets an admin select the correct SDG (or "none"), which becomes the new `llm_sdg_primary` at 100% confidence, with `llm_reviewed_by`/`llm_reviewed_at` recording who/when - directly answers the evaluator's transparency criterion (4.3) about human oversight of automated decisions. Verified end-to-end against the local Docker stack: lowered a real classification's confidence to simulate the low-confidence case, confirmed it appeared in the review queue, submitted a correction (SDG 3 → SDG 2), confirmed the database updated correctly (100% confidence, reviewer username and timestamp recorded) and the item disappeared from the pending queue; unauthenticated request correctly rejected (401). Test data reverted after verification.
