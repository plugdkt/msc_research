...
- [x] **SDG-06c (Batch Auto-Classify)**: Provide a one-click batch auto-classification tool for admins to automatically tag unclassified publications using weighted keyword scoring. *Done 2026-08-19, on explicit user request to unblock the earlier deferral. New `admin/auto_classify_sdgs.php` (`action=list` / `action=process`) + a JS driver in `admin/sdg_import.php` process one publication per HTTP request in a browser-driven sequential loop, not one big server-side loop - avoids the IIS FastCGI timeout risk flagged in the original deferral (one Scopus Abstract Retrieval API call per publication, no batch endpoint exists) while giving a live progress bar, per-item log, and a Stop button. Each publication commits independently, so the whole batch is resumable if interrupted. Writes `sdg_primary`/`sdg_secondary`/`sdg_tertiary`/`sdg_rationale` unattended only when a match score clears `MIN_AUTO_APPLY_SCORE` (a deliberate safety threshold, not part of msc_sdgs's own algorithm, added to protect data accuracy from low-confidence keyword coincidences); below it, the publication is left Unclassified for manual review via SDG-06b instead. Never overwrites an existing SDG tag.
  **Threshold history**: 0.4 (initial) → **1.0** (2026-08-19, server-side review: "≥1.0 or full-phrase match", since this data is published faculty-wide) → **>0.5, strictly greater than** (2026-08-20, per faculty leadership decision relayed by the user: "1.0 คือคะแนนเต็ม", so more than half of a full-phrase match is confident enough). The comparison changed from `<` (allowing `>=`) to an explicit `> MIN_AUTO_APPLY_SCORE` check to match "มากกว่า" (strictly more than), not "at least."
...
> **Spec only, written 2026-08-20** at the user's request, not yet implemented. The two design decisions below (RCR-05, RCR-06) were open pending user confirmation and were confirmed on 2026-08-20 - both are now finalized requirements. See ROADMAP.md's Phase 7 "Design notes" for the alternatives considered (OpenAlex, FWCI/SciVal) and why NIH iCite was chosen despite needing a PMID.

- [ ] **RCR-01 (DOI → PMID Resolution)**: System resolves a publication's DOI to its PubMed ID (PMID) via the NCBI ID Converter API (`https://www.ncbi.nlm.nih.gov/pmc/utils/idconv/v1.0/`, free, no API key required), storing the resolved PMID on the publication so repeat lookups aren't needed. A publication with no DOI, or whose DOI doesn't resolve to a PMID (common - PubMed only indexes biomedical/health literature, so non-MEDLINE-indexed journals genuinely have no PMID), is left with a null PMID rather than treated as an error.
- [ ] **RCR-02 (iCite Fetch)**: For publications with a resolved PMID, system fetches the Relative Citation Ratio (`relative_citation_ratio`) and NIH percentile (`nih_percentile`) from the NIH iCite API (`https://icite.od.nih.gov/api/pubs?pmids=...`, free, no API key required, accepts a batch of PMIDs per call) and stores them on the publication.
- [ ] **RCR-03 (No Live Fetch On Page Render)**: PMID resolution and RCR fetching only ever happen via an explicit admin-triggered action, never inline during a public page render - the same "Live Scopus API calls on page render" rule already in Out of Scope, extended to these two new external calls for the same reason (public-site uptime/latency must not depend on an external API, and repeated fetches per page view waste quota).
- [ ] **RCR-04 (Graceful Absence)**: A publication with no resolvable PMID, or a PMID iCite has no record for, displays "RCR not available" (or is simply omitted where a per-publication badge doesn't apply) rather than blank or an error, and is skipped without failing any batch operation - the same graceful-absence pattern already used for missing Scopus abstract/keywords (SDG-06b) and Unclassified SDGs (SDG-03).
- [ ] **RCR-05 (Display Location)**: *(decided 2026-08-20)* RCR/percentile is shown in all three candidate locations, not just one - the Reports page (aggregate/summary), the researcher profile page (per-publication), and publication list badges (index.php/publications_search.php). Full public display; not limited to the admin panel.
- [ ] **RCR-06 (Fetch Trigger)**: *(decided 2026-08-20)* Both candidate mechanisms are implemented. The primary/main mechanism is a batch tool mirroring SDG-06c's Auto-Classify (one-click, processes all publications, with a manual "Refresh" re-run since RCR changes over time as citations accrue - unlike SDG mapping, this is never a one-time classification). Secondary/supplementary is an on-demand per-publication fetch mirroring SDG-06b's Suggest SDGs, for resolving or refreshing a single publication while an admin has it open.
- [ ] **RCR-07 (Refresh Support)**: Per RCR-06, admin can re-run RCR/percentile fetching for already-resolved publications - via the batch tool's "Refresh" action (all/stale publications at once) or via the on-demand fetch (one publication at a time) - to pick up iCite's periodic recomputation as new citing literature is indexed. Unlike SDG-06c's "never overwrite an existing tag" guard, a refresh here is always safe to overwrite - RCR/percentile are machine-computed only, nothing in this system ever lets a human hand-enter or curate them, so there's no risk of clobbering a curated value.
