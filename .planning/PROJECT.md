...
- **[แก้ไขแล้ว 2026-08-20 — ช่องโหว่ CSRF]** endpoint แบบ batch/on-demand ทั้งหมดที่สร้างขึ้นใน Phase 6-9 (`admin/auto_classify_sdgs.php`, `admin/suggest_sdgs.php`, `admin/classify_topics.php`, `admin/fetch_rcr.php`, `admin/fetch_quartiles.php`) เป็น GET request ที่เช็คแค่ session login ไม่มีการป้องกัน CSRF — admin ที่ login ค้างอยู่คลิกลิงก์/เปิดหน้าเว็บที่ถูกออกแบบมาไม่ดีสามารถถูกสั่งให้เขียนข้อมูลจริงโดยไม่ตั้งใจได้ (เขียนทับ SDG/Topic/RCR/Quartile ของผลงานวิจัยจริง) แก้แล้วด้วย per-session CSRF token (`get_csrf_token()`/`verify_csrf_token()` ใน `includes/functions.php`, ส่งผ่าน header `X-CSRF-Token` แทน query string เพื่อไม่ให้หลุดผ่าน Referer log) เช็คเฉพาะ action ที่เขียนข้อมูลจริง (`action=process`) ไม่เช็ค `action=list` เพราะเป็น read-only — ทดสอบยืนยันแล้วว่าไม่มี token = 403, token ถูก = ผ่าน, token ผิด = 403 ในทุก endpoint ที่แก้
...
| Phase 9: เลิกใช้ SCImago/SJR Quartile ทั้งหมด เปลี่ยนเป็น Scopus CiteScore Quartile (คำนวณจาก percentile ที่ดึงได้ผ่าน Serial Title API ด้วย API key เดิม ไม่ต้องขอ key ใหม่) เป็นแหล่งเดียว | ผู้บริหารยืนยันโดยตรง ("ใช้ scopus quartile เป็นหลักครับ SCImago ไม่ได้ใช้แล้ว") หลังจากตรวจสอบว่าไฟล์ Scopus Source List ที่ผู้ใช้ส่งมาไม่มีข้อมูล Quartile แต่ API ที่มีอยู่แล้วดึง percentile ได้จริงผ่าน view=CITESCORE (ยืนยันสูตรคำนวณตรงกับนิยามทางการของ Scopus เองผ่านการค้นหาเว็บ ไม่ใช่การประมาณเอง) | ✓ Good — implement และ verify กับ Scopus API จริงบน local Docker stack แล้ว 2026-08-20 |
